...
def get_plm_representation(model_type,fasta_file_wo_gap,fasta_file_std_gap,layers, domain=False):

    file_location  = os.path.dirname(fasta_file_wo_gap)
    if domain:
        file_name = f'{file_location}/{model_type}_domain.pkl'
    else:
        file_name = f'{file_location}/{model_type}.pkl'
    if os.path.exists(file_name):
       print(model_type,fasta_file_wo_gap)
    ...
    if model_type == 'esmc':
        ...
        from esm1.esm.models.esmc import ESMC        
        
        hugging_token = open('../secrets.txt').read().strip()
        ...
        if skipped > 0:
            ...
            print(f"Skipping ESMC for the pfam {file_location.split('/')[-1]}")
            return
        ...
        PLM_dataset = utils_embeddings.get_esmc_embedding(
            fasta_file_wo_gap, esmc_model, output_layers
        )
    
        
    with open(file_name, "wb") as f:
        pickle.dump(PLM_dataset, f)
