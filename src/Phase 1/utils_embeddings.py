...
# Our Code
def get_esmc_embedding(fasta_file_name, model, output_layers, device=None):
    # Read fasta sequences
    seq_dict = read_fasta(fasta_file_name)  # dict {id: seq}
    seq_labels = list(seq_dict.keys())
    seq_seqs   = list(seq_dict.values())

    # Call Forge API
    outputs = []
    for seq in seq_seqs:
        outputs.append(embed_sequence(model,seq))

    # outputs = batch_embed(model, seq_seqs)

    plm_embedding = []
    valid_labels = []

    for label, output in zip(seq_labels, outputs):
        if isinstance(output, ESMProteinError):
            print(f"[WARN] Failed to embed {label}, skipping...")
            print(output)
            
            continue

        # hidden_states: [layers, seq_len, dim]
        # take mean over seq_len for each layer
        mean_per_layer = output.hidden_states.mean(dim=-2).to(torch.float32).cpu().numpy().squeeze()
#        torch.mean(output.hidden_states, dim=-2).squeeze(0).cpu().numpy()
        plm_embedding.append(mean_per_layer)
        valid_labels.append(label)

    # [num_sequences, num_layers, dim]
    plm_embedding_tensor = torch.tensor(np.array(plm_embedding))
    print(f"ESM-C embeddings extracted, shape {plm_embedding_tensor.shape}")

    return PLMEmb_Dataset(valid_labels, plm_embedding_tensor)
