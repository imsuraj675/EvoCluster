...
def load_plm_dataset(pfam, model_type):
    fasta_file_wo_gap  = f"../data/{pfam}/{pfam}_domain.aln"
    fasta_file_std_gap = f"../data/{pfam}/{pfam}_domain.aln"
    layers = "all"

    # Call get_plm_representation -> returns PLMEmb_Dataset
    plmdata = get_plm_representation(model_type, fasta_file_wo_gap, fasta_file_std_gap, layers, True)
    return plmdata
...
def main():
    ...
    results_dir = "results_domain_interfamily"
