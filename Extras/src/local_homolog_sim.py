...
def help():
    ...
    print('python local_homolog_sim.py -a alignment file -m model name -c Y')
    exit()
...
def local_sim_analysis(fasta_aln_file,model_type,k,PLM_dataset=None):
    ...
    if PLM_dataset is None:
        layers = [model_layer_plm_repr[model_type]]
        print(f"Getting pLM representation for protein family {base_file_name} "
              f"from model {utils_get_embeddings.full_model_name[model_type]}")
        PLM_dataset = utils_get_embeddings.get_plm_representation(
            model_type, fasta_file_wo_gap, fasta_file_std_gap, layers
        )
        print(f"pLM embedding created for {len(PLM_dataset[:][0])} sequences "
              f"each of size {PLM_dataset[0][1].shape}")
    else:
        print("Using provided PLM_dataset (skipping recomputation).")
